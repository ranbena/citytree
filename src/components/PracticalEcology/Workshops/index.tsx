  ...
  const renderPreviousLink = () => {
    ...
    return (
      <Prev onClick={() => setTabKey(prevKey)} $hidden={tabKey === 'prosperity'}>
        <Prefix>
          <FormattedMessage id="practical-ecology.workshops.prev" />:
        </Prefix>{' '}
        {getTitle(prevKey)}
      </Prev>
    );
  ...
  return (
    <Wrapper>
      <Container>
        ...
        <Subtitle>
          <FormattedMessage id="practical-ecology.workshops.subtitle" />
        </Subtitle>
            ...
            {tabKeys.map((key) => (
              <Nav.Item key={key}>
                <Nav.Link eventKey={key} onClick={() => setTabKey(key)} active={tabKey === key}>
                  <FormattedMessage id="practical-ecology.workshops.timeof" /> {getTitle(key)}
                </Nav.Link>
              </Nav.Item>
            ))}
          </Tabs>
